...
const PokemonBattle = () => {
  ...
  useEffect(() => {
    ...
    if (allPokemon.length === 0) {
      axios
        .get("https://pokeapi.co/api/v2/pokemon?limit=10000") // Very high limit to get all Pokémon
        .then(async (res) => {
          const details = await Promise.all(
            res.data.results.map(async (pokemon) => {
              ...
              return {
                ...
                stats: {
                  hp: detail.data.stats.find((stat) => stat.stat.name === "hp")
                    .base_stat,
                  attack: detail.data.stats.find(
                    (stat) => stat.stat.name === "attack"
                  ).base_stat,
                  defense: detail.data.stats.find(
                    (stat) => stat.stat.name === "defense"
                  ).base_stat,
                  speed: detail.data.stats.find(
                    (stat) => stat.stat.name === "speed"
                  ).base_stat,
                },
                height: detail.data.height / 10,
                weight: detail.data.weight / 10,
                abilities: detail.data.abilities.map(
                  (ability) => ability.ability.name
                ),
                animatedSprite:
                  detail.data.sprites.versions?.["generation-v"]?.[
                    "black-white"
  ...
  const simulateBattle = () => {
    ...
    setTimeout(() => {
      ...
      const battleRecord = {
        ...
        winner,
        result:
          winner === "hero" ? "win" : winner === "opponent" ? "lose" : "draw",
        ...
        <div className="w-full md:w-[300px] pt-20 relative mt-10 md:order-first">
          {hero ? (
            ...
            <div className="w-40 h-40 flex items-center justify-center bg-gray-100 rounded-xl shadow mb-4">
              <img
                src={placeholderImg}
                alt="Select your hero"
                className="h-24 opacity-40"
              />
            </div>
          )}
          {!hero && <div className="text-center font-semibold text-lg"></div>}
        </div>
        ...
        <div className="flex-1 flex flex-col items-center">
          {battleLoading ? (
            ...
            <div className="flex flex-col items-center min-w-[300px] py-12">
              ...
              <div className="text-2xl font-bold text-[#FF5350] mb-2">
                Battle Loading...
              </div>
            ...
            step === 1 ? (
              <>
                <div className="mb-4 text-center font-bold text-xl mt-20">
                  Select Your Hero
                </div>
                {team.length === 0 ? (
                  <div className="text-gray-500">
                    Your team is empty. Add Pokémon to your team first!
                  </div>
                ) : (
                  <div className="grid grid-cols-3 gap-4">
                    {team.map((p) => (
                      <button
                        key={p.id}
                        className={`flex flex-col items-center border-2 rounded-lg p-2 transition-all hover:border-[#FF5350] ${
                          hero?.id === p.id
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          {allPokemon
                            ...
                            .map((p) => {
                              const isHero =
                                hero &&
                                (p.id === hero.id || p.id === hero.pokemonId);
                              return (
                                <button
                                  key={p.id}
                                  disabled={isHero}
                                  ...
                                  onClick={() => !isHero && setOpponent(p)}
                                  title={
                                    isHero ? "This is your selected hero" : ""
                                  }
                                >
                                  <img
                                    src={p.image}
            ...
            <div className="flex flex-col items-center min-w-[300px] mb-6 mt-">
              ...
              <table className="mb-6 w-full max-w-md text-center ">
                <thead>
                  <tr>
                    ...
                    <th className="py-2">
                      {hero ? capitalize(hero.name) : "Hero"}
                    </th>
                    <th className="py-2">
                      {opponent ? capitalize(opponent.name) : "Opponent"}
                    </th>
              ...
              <div className="text-xl font-bold mb-4">
                {battleResult.winner === "hero" && hero && (
                  <span className="text-green-600">
                    {capitalize(hero.name)} Wins!
                  </span>
                )}
                {battleResult.winner === "opponent" && opponent && (
                  <span className="text-red-600">
                    {capitalize(opponent.name)} Wins!
                  </span>
                )}
